...
const WCManager = {
  ...
  async run(files, onLog, onUrl) {
    if (this._running) { await this.killDev(); }
    this._running = true;
    ...
    if (!validation.valid) {
      ...
      try {
        ...
        if (fixResult.fixed) {
          ...
          if (!recheck.valid) onLog(`Ainda restam ${recheck.errors.length} erro(s) — montando mesmo assim`, "warn");
        } else { onLog("Auto-fix nao conseguiu corrigir — montando com erros", "warn"); }
      } catch { onLog("Auto-fix indisponivel — montando com erros", "warn"); }
    } else { onLog("Sintaxe OK!", "success"); }
    ...
    } catch (e) { console.log("[Zero AUDIT] Erro lendo vite.config.js do WC:", e.message); }
    ...
    try {
      ...
      const lsChunks = [];
      await ls.output.pipeTo(new WritableStream({ write(chunk) { lsChunks.push(chunk); } }));
      await ls.exit;
      console.log(lsChunks.join("") || "(vazio — Babel nao encontrado)");
      console.log("[Zero AUDIT] === npm ls @vitejs/plugin-react --depth=1 ===");
      const ls2 = await wc.spawn("npm", ["ls", "@vitejs/plugin-react", "--depth=1"]);
      const ls2Chunks = [];
      await ls2.output.pipeTo(new WritableStream({ write(chunk) { ls2Chunks.push(chunk); } }));
      await ls2.exit;
      ...
      const ls3Chunks = [];
      await ls3.output.pipeTo(new WritableStream({ write(chunk) { ls3Chunks.push(chunk); } }));
      await ls3.exit;
      console.log(ls3Chunks.join("") || "(vazio — plugin-react-swc nao encontrado)");
    } catch (e) { console.log("[Zero AUDIT] Erro no npm ls:", e.message); }
    ...
    } catch (e) { console.log("[Zero AUDIT] Erro lendo package.json do WC:", e.message); }
  ...
  async _startViteWithTimeout(wc, files, onLog, onUrl, isRetry = false) {
    ...
    this.devProcess.output.pipeTo(new WritableStream({ write(chunk) { onLog(chunk); } }));
